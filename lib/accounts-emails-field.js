...
getEmailsFromService = function(service_name, service){
    ...
    if (service.emailAddress)
      emails = [ { address: service.emailAddress } ];
    if (service.emails)
    ...
    return emails.map(function (email) {
      ...
      if (_.indexOf(whitelistedServices, service_name) > -1)
          verified = true;
      else if (email.verified) // Meteor developer account
          verified = true;
      else if (service.verified_email)
          verified = true;

      return {address: email.address, verified: verified};
...
updateEmails = function(info) {
    ...
    _.each(user.services, function(service, service_name) {
        if (service_name === 'resume' || service_name === 'email' || service_name === 'password')
            return;
        var serviceEmails = getEmailsFromService(service_name, service);
